...
func NewSFBCollector(netns map[string][]rtnetlink.LinkMessage, fqcodellog *slog.Logger) (prometheus.Collector, error) {
...
func (qc *SFBCollector) Describe(ch chan<- *prometheus.Desc) {
	ds := []*prometheus.Desc{
	}
...
func (qc *SFBCollector) Collect(ch chan<- prometheus.Metric) {
