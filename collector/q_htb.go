...
func NewHtbCollector(netns map[string][]rtnetlink.LinkMessage, log *slog.Logger) (TcSubCollector, error) {
	// Setup logger for qdisc collector
	log = log.With("collector", "htb")
	log.Debug("making htb collector")
...
func (col *HtbCollector) Collect(ch chan<- prometheus.Metric, objects map[string]map[string][]tc.Object) {
