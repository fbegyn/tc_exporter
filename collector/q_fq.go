...
func NewFqCollector(netns map[string][]rtnetlink.LinkMessage, log *slog.Logger) (TcSubCollector, error) {
	// Setup logger for qdisc collector
	log = log.With("collector", "fq")
	log.Debug("making fq collector")
...
func (col *FqCollector) Collect(ch chan<- prometheus.Metric, objects map[string]map[string][]tc.Object) {
	...
	for ns, devices := range col.netns {
		for _, interf := range devices {
			...
			for _, qd := range objects[ns]["qdisc"] {
				if qd.Fq == nil || qd.Msg.Ifindex != interf.Index {
					continue
				}
