...
func NewCodelCollector(netns map[string][]rtnetlink.LinkMessage, fqcodellog *slog.Logger) (prometheus.Collector, error) {
...
func (qc *CodelCollector) Describe(ch chan<- *prometheus.Desc) {
	ds := []*prometheus.Desc{
	}
...
func (qc *CodelCollector) Collect(ch chan<- prometheus.Metric) {
