...
func NewChokeCollector(netns map[string][]rtnetlink.LinkMessage, fqcodellog *slog.Logger) (prometheus.Collector, error) {
...
func (qc *ChokeCollector) Describe(ch chan<- *prometheus.Desc) {
	ds := []*prometheus.Desc{
	}
...
func (qc *ChokeCollector) Collect(ch chan<- prometheus.Metric) {
