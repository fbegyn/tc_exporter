...
func NewSFQCollector(netns map[string][]rtnetlink.LinkMessage, fqcodellog *slog.Logger) (prometheus.Collector, error) {
...
func (qc *SFQCollector) Describe(ch chan<- *prometheus.Desc) {
	ds := []*prometheus.Desc{
	}
...
func (qc *SFQCollector) Collect(ch chan<- prometheus.Metric) {
